...
// Функция которая проверяет может ли браузер поддерживать разрешение webp; // Если убрать скрипт, то это добавить +5 баллов на PageSpeed

// Добавление data-srcset в source конструкции picture
// window.addEventListener("DOMContentLoaded", getArrForPicture);
getArrForPicture();
async function getArrForPicture() {
    let arrOfSource = document.querySelectorAll('._wrap-to-animation source');
    let arrOfImgInPicture = document.querySelectorAll('._wrap-to-animation img');
    for (let index = 0; index < arrOfSource.length; index++) {
        const element = arrOfSource[index];
        let protoElement = arrOfImgInPicture[index].dataset.src;
        element.dataset.srcset = lookForBitOfString(protoElement);
    }
}

function lookForBitOfString(str, whatWeLookFor = 'jpg' || 'jpeg' || 'png' || 'gif') {
    let placeLocation = str.indexOf(whatWeLookFor, 0);
    return cutBitOfString(str, placeLocation);
}
function cutBitOfString(str, end, start = 0) {
    let cuttedString = str.slice(start, end);
    return cuttedString + "webp";
}
//===============================================================

// Lazy Load function
// setTimeout(plugInLazzy, 300);
plugInLazzy();

async function plugInLazzy() {
    ...
    if (lazyImages.length > 0) {
        lazyImages.forEach(img => {
            if (img.dataset.src || img.dataset.srcset) {
                ...
                lazyScrollCheck(lazyImagesPositions, windowHeight);
            }
        });
    }
    ...
    function lazyScroll() {
        ...
        if (document.querySelectorAll('img[data-src],source[data-srcset]').length > 0) {
            lazyScrollCheck();
        } else window.removeEventListener('scroll', lazyScroll);
    }
    ...
    function lazyScrollCheck() {
        ...
        if (imgIndex >= 0) {
            if (lazyImages[imgIndex].dataset.src) {
                ...
                if (lazyImages[imgIndex].closest('._wrap-to-animation')) { lazyImages[imgIndex].closest('._wrap-to-animation').classList.remove('_wrap-to-animation'); }
...
function createDobleMenu() {
	let menuResourse = document.querySelector('.menu__body');
	let menuInclude = document.querySelector('.menu__body2');
	menuInclude.innerHTML = menuResourse.innerHTML;
	old_current_link = document.querySelectorAll('._goto-block._active');
	link = document.querySelectorAll('._goto-block');
};
// Чек лист по Скроллу:
...
if (link) {
	setTimeout(pluginClickScroll, 500);
	function pluginClickScroll() {
		let blocks = [];

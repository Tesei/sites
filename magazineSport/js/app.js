...
// todo Если использовать функцию _slideToggle(имяПеременной), то блок, которому присвоена переменная будет выезжать как слайдер
// ? для того чтобы при загрузке страницы блок не открывался, ему нужно присвоить nameBlock.hidden = true;
...
if (document.querySelector('.mainslider')) {
	...
	// todo Тут (в скобочках) вставляем имя класса слайдера, который нужно активизировать
...
// Вывод маски галереи на слайдер: ----------------------------

if (document.querySelector('.images-product__mainslider')) { // Проверяем на наличие слайдера, по которому будем щёлкать

	let mainSliderItems = document.getElementsByClassName('images-product__mainslide');

	for (let index = 0; index < mainSliderItems.length; index++) {
		const slideItem = mainSliderItems[index];

		slideItem.addEventListener("click", function (e) {			

			let collectionImages = document.querySelectorAll('.images-product__mainslide'); // Название класса индивидуальной оболчки слайдов
			let numberActiv = 0;
	
			for (let index = 0; index < collectionImages.length; index++) {
				const element = collectionImages[index];
			...
			let itemGalleryToShow = document.querySelectorAll('.product__gallery-place')[numberActiv]; // название класса индивидуальной оболочки картинки галереи 
			itemGalleryToShow.click();	
...
da.init();
// todo Тут указываем класс объекта, для которого выводить подсказку и текст самой подсказки
// tippy('.tippy', {
// 	content: "I'm a Tippy tooltip!",
// });


// Создание возможности выдвижения меню на мобильных устройствах
if (isMobile.any()) {
	...
	// создание способности выдвижения субМеню
	for (let index = 0; index < menuParents.length; index++) {
		const menuParent = menuParents[index];
...
// Открытие и закрытие меню блока меню, слева
let menuPageBurger = document.querySelector('.menu-page__lines');
let menuPageBody = document.querySelector('.menu-page__body');

// чтобы изначально блок был свёрнут
...
// Закрытие и раскрытие фильтра товаров на мобильных устройствах
if (isMobile.any()) {
...
// смена построения сетки товаров

if (document.querySelector('.view-catalog')) {

	let viewGrid = document.querySelector('.view-catalog__item_grid');	
	let viewList = document.querySelector('.view-catalog__item_list');
	let catalogInLine = document.querySelector('.items-products-inline');
	let catalogGrid = document.querySelector('.items-products');

	viewGrid.addEventListener('click', function (e) {
		viewGrid.classList.toggle('_active');
		viewList.classList.toggle('_active');

		catalogInLine.classList.toggle('_active');
		catalogGrid.classList.toggle('_active');
	});
		...
		for (let index = 0; index < inputs.length; index++) {
			...
			if (input.classList.contains('_date')) {
				const calendarItem = datepicker(input, {
					// todo Тут можно настроить календарь
...
//RANGE
// todo В строке ниже, в скобочках указываем имя класса
const priceSlider = document.querySelector('.price-filter__slider');
if (priceSlider) {

	// ? Строки ниже дают подписи, через атрибуты, сверху, к коненым точкам линии регулировки
	...
	// ? Строки ниже связывают допазон регулировки с инпутами
